...
type TData = TGarageDeficiencies | undefined;

interface IProps {
  path: string;
  stepDescription: string;
  data: TData;
  onSave: (data: TGarageDeficiencies, path: string) => void;
...
const getFormDefaultValues = (data: TData) => {
  return {
    garageDeficiencies: data?.garageDeficiencies,
  };
};

const GarageDeficienciesForm = ({
  path,
  stepDescription,
  data,
  onSave,
  ...
  const form = useForm<TGarageDeficiencies>({
    resolver: zodResolver(garageDeficienciesObjSchema),
    defaultValues: getFormDefaultValues(data),
  });

  const onSubmit: SubmitHandler<TGarageDeficiencies> = async (data) => {
    onSave(data, path);
    onBackward();
    onNextStep(BUILDINGS_INNER_SEGMENT.GARAGE);
  };

  const controlBasePathName = 'garageDeficiencies';
  ...
  const handleEditItem = async (index: number) => {
    ...
    onNextPath(nextPath);
    onNextStep(BUILDINGS_INNER_SEGMENT.GARAGE_DEFICIENCY);
  };

  const handleAddItem = async (index: number) => {
    fieldsArray.append({} as TGarageDeficienciesItem);
  ...
  return (
    <div className="w-full max-w-[612px] flex-col items-center">
      <header className="mb-[60px]">
        <ReportSectionHeader
          title={calcInnerSegmentTitle(BUILDINGS_INNER_SEGMENT.GARAGE_DEFICIENCIES)}
        ...
        <Form {...form}>
          ...
          >
            {fieldsArray.fields.map((arrayItem, index) => {
              ...
              const errorMessage = form.formState.errors?.garageDeficiencies?.[index]
                ? isEmpty
                  ? 'Required'
                  : 'Contains errors'
                : '';
              ...
              return (
                <FormArrayControl
                  key={arrayItem.id}
                  title={`Garage Deficiency ${calcArrayItemTitle(arrayItem.garageDeficiencyName)}`}
...
export default GarageDeficienciesForm;
