...
import {
  mechanicalDeficienciesObjSchema,
...
type TData = TMechanicalDeficiencies | undefined;

interface IProps {
  path: string;
  stepDescription: string;
  data: TData;
  onSave: (data: TMechanicalDeficiencies, path: string) => Promise<boolean>;
...
const getFormDefaultValues = (data: TData) => {
  return {
    mechanicalDeficiencies: data?.mechanicalDeficiencies,
  };
};

const MechanicalDeficienciesForm = ({
  path,
  stepDescription,
  data,
  onSave,
  ...
  const form = useForm<TMechanicalDeficiencies>({
    resolver: zodResolver(mechanicalDeficienciesObjSchema),
    defaultValues: getFormDefaultValues(data),
  });

  const onSubmit: SubmitHandler<TMechanicalDeficiencies> = async (data) => {
  ...
  const controlBasePathName = 'mechanicalDeficiencies';
  ...
  const handleEditItem = async (index: number) => {
    ...
    onNextPath(nextPath);
    onNextStep(BUILDINGS_INNER_SEGMENT.MECHANICAL_DEFICIENCY);
  };

  const handleAddItem = async (index: number) => {
    fieldsArray.append({} as TMechanicalDeficiencies['mechanicalDeficiencies'][0]);
  ...
  return (
    <div className="w-full max-w-[612px] flex-col items-center">
      <header className="mb-[60px]">
        <ReportSectionHeader
          title={calcInnerSegmentTitle(BUILDINGS_INNER_SEGMENT.MECHANICAL_DEFICIENCIES)}
        ...
        <Form {...form}>
          ...
          >
            {fieldsArray.fields.map((arrayItem, index) => {
              ...
              const errorMessage = form.formState.errors?.mechanicalDeficiencies?.[index]
                ? isEmpty
                  ? 'Required'
                  : 'Contains errors'
                : '';
              ...
              return (
                <FormArrayControl
                  key={arrayItem.id}
                  title={`Mechanical Deficiency ${calcArrayItemTitle(arrayItem.mechanicalDeficiencyName)}`}
...
export default MechanicalDeficienciesForm;
