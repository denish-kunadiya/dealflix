...
import { mechanicalUpdatesObjSchema, TMechanicalUpdates } from '@/utils/api/schemas/report';
...
type TData = TMechanicalUpdates | undefined;

interface IProps {
  path: string;
  stepDescription: string;
  data: TData;
  onSave: (data: TMechanicalUpdates, path: string) => Promise<boolean>;
...
const getFormDefaultValues = (data: TData) => {
  return {
    mechanicalUpdates: data?.mechanicalUpdates,
  };
};

const MechanicalUpdatesForm = ({
  path,
  stepDescription,
  data,
  onSave,
  ...
  const form = useForm<TMechanicalUpdates>({
    resolver: zodResolver(mechanicalUpdatesObjSchema),
    defaultValues: getFormDefaultValues(data),
  });

  const onSubmit: SubmitHandler<TMechanicalUpdates> = async (data) => {
  ...
  const controlBasePathName = 'mechanicalUpdates';
  ...
  const handleEditItem = async (index: number) => {
    ...
    onNextPath(nextPath);
    onNextStep(BUILDINGS_INNER_SEGMENT.MECHANICAL_UPDATE);
  };

  const handleAddItem = async (index: number) => {
    fieldsArray.append({} as TMechanicalUpdates['mechanicalUpdates'][0]);
  ...
  return (
    <div className="w-full max-w-[612px] flex-col items-center">
      <header className="mb-[60px]">
        <ReportSectionHeader
          title={calcInnerSegmentTitle(BUILDINGS_INNER_SEGMENT.MECHANICAL_UPDATES)}
        ...
        <Form {...form}>
          ...
          >
            {fieldsArray.fields.map((arrayItem, index) => {
              ...
              const errorMessage = form.formState.errors?.mechanicalUpdates?.[index]
                ? isEmpty
                  ? 'Required'
                  : 'Contains errors'
                : '';
              ...
              return (
                <FormArrayControl
                  key={arrayItem.id}
                  title={`Mechanical Update ${calcArrayItemTitle(arrayItem.mechanicalUpdateComponent)}`}
...
export default MechanicalUpdatesForm;
