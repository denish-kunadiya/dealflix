...
import { exteriorUpdatesObjSchema, TExteriorUpdates } from '@/utils/api/schemas/report';
...
type TData = TExteriorUpdates | undefined;

interface IProps {
  path: string;
  stepDescription: string;
  data: TData;
  onSave: (data: TExteriorUpdates, path: string) => Promise<boolean>;
...
const getFormDefaultValues = (data: TData) => {
  return {
    exteriorUpdates: data?.exteriorUpdates,
  };
};

const ExteriorUpdatesForm = ({
  path,
  stepDescription,
  data,
  onSave,
  ...
  const form = useForm<TExteriorUpdates>({
    resolver: zodResolver(exteriorUpdatesObjSchema),
    defaultValues: getFormDefaultValues(data),
  });

  const onSubmit: SubmitHandler<TExteriorUpdates> = async (data) => {
  ...
  const controlBasePathName = 'exteriorUpdates';
  ...
  const handleEditItem = async (index: number) => {
    ...
    onNextPath(nextPath);
    onNextStep(BUILDINGS_INNER_SEGMENT.EXTERIOR_UPDATE);
  };

  const handleAddItem = async (index: number) => {
    fieldsArray.append({} as TExteriorUpdates['exteriorUpdates'][0]);
  ...
  return (
    <div className="w-full max-w-[612px] flex-col items-center">
      <header className="mb-[60px]">
        <ReportSectionHeader
          title={calcInnerSegmentTitle(BUILDINGS_INNER_SEGMENT.EXTERIOR_UPDATES)}
        ...
        <Form {...form}>
          ...
          >
            {fieldsArray.fields.map((arrayItem, index) => {
              ...
              const errorMessage = form.formState.errors?.exteriorUpdates?.[index]
                ? isEmpty
                  ? 'Required'
                  : 'Contains errors'
                : '';
              ...
              return (
                <FormArrayControl
                  key={arrayItem.id}
                  title={`Exterior Update ${calcArrayItemTitle(arrayItem.exteriorUpdatedComponent)}`}
...
export default ExteriorUpdatesForm;
