...
import { levelsObjSchema, TLevels, TLevelsItem } from '@/utils/api/schemas/report';
...
type TData = TLevels | undefined;

interface IProps {
  path: string;
  stepDescription: string;
  data: TData;
  onSave: (data: TLevels, path: string) => Promise<boolean>;
...
const LevelsForm = ({
  ...
  const form = useForm<TLevels>({
    resolver: zodResolver(levelsObjSchema),
    defaultValues: getFormDefaultValues(data),
  });

  const onSubmit: SubmitHandler<TLevels> = async (data) => {
  ...
  const controlBasePathName = 'levels';
  ...
  const handleAddItem = async (index: number) => {
    fieldsArray.append({} as TLevelsItem);
  ...
  return (
    <div className="w-full max-w-[612px] flex-col items-center">
      <header className="mb-[60px]">
        <ReportSectionHeader
          title={calcInnerSegmentTitle(BUILDINGS_INNER_SEGMENT.LEVELS)}
        ...
        <Form {...form}>
          ...
          >
            {fieldsArray.fields.map((arrayItem, index) => {
              ...
              const errorMessage = form.formState.errors?.levels?.[index]
                ? isEmpty
                  ? 'Required'
                  : 'Contains errors'
                : '';
              ...
              return (
                <FormArrayControl
                  key={arrayItem.id}
                  title={`Level ${calcArrayItemTitle(arrayItem.levelNumber?.toString())}`}
