...
import { GAS_TYPES } from '@/utils/api/schemas/constants';
import { siteUtilityShema, TGasServices } from '@/utils/api/schemas/report';

import ReportSectionHeader from '../report-section-header';

type TData = TGasServices | undefined;
...
const GasServicesForm = ({
  ...
  const form = useForm<TGasServices>({
    resolver: zodResolver(validationSchema),
    defaultValues: getFormDefaultValues(data),
  });

  const controlBasePathName = 'gasServices';
  const fieldsArray = useFieldArray({
    control: form.control,
    name: controlBasePathName,
  });

  const onSubmit: SubmitHandler<TGasServices> = async (data) => {
    onSave(data);
  ...
  return (
    <div className="w-full max-w-[612px] flex-col items-center">
      <header className="mb-[60px]">
        <ReportSectionHeader
          title={calcInnerSegmentTitle(SITE_INNER_SEGMENT.GAS_SERVICES)}
            ...
            {fieldsArray.fields.map((field, index) => {
              return (
                <Fragment key={field.id}>
                  <FormField
                    control={form.control}
                    name={`${controlBasePathName}.${index}.gasType`}
                    render={({ field }) => (
                      <FormItem>
...
export default GasServicesForm;
